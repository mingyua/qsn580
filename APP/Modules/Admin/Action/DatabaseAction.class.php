   ...
   public function index() {
	   $DataDir = "backup/";
        ...
        if (!empty($_GET['Action'])) {
			import('Class.MySQLReback',APP_PATH);
            $config = array(
                'host' => C('DB_HOST'),
                'port' => C('DB_PORT'),
                'userName' => C('DB_USER'),
            ...
            if ($_GET['Action'] == 'backup') {
                $mr->backup();
                echo "<script>alert('添加备份成功！');document.location.href='" . U("Database/index") . "'</script>";
//                $this->success( '数据库备份成功！');
            } elseif ($_GET['Action'] == 'RL') {
                $mr->recover($_GET['File']);
                echo "<script>document.location.href='" . U("Database/index") . "'</script>";
