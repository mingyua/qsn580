...
<?php echo ($nav); ?>
<div class="pd-20">
 	<form action="<?php echo U(GROUP_NAME.'/Article/add');?>" method="post" enctype="multipart/form-data"> 
   ...
   <?php if(is_array($cate)): foreach($cate as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["html"]); echo ($v["name"]); ?></option><?php endforeach; endif; ?>
    ...
    <td><input name="title" type="text" class="input-text" style="width:500px"  /></td>
    <tr>
    <th class="text-r"><?php echo C('news_title');?></th>
    <td><input name="retitle" type="text" class="input-text" style="width:500px"  /></td>
    </tr>
    <tr><th class="text-r"><?php echo C('fromlink');?></th><td><input name="url" type="text" class="input-text"  style="width:500px" value="<?php echo ($config['site_url']); ?>" />&nbsp;&nbsp;如：Http://www.gylongteng.com</td></tr>
    <tr><th class="text-r"><?php echo C('thumb');?></th><td>
	  ...
	  </span><span class="ml-10" style="float:left;" ><img id="img1" src="" style="display: block;" /></span>
           </td>
          </tr>
        </tbody>
        </table>
    </td></tr>
    <tr id="last"><th class="text-r"><?php echo C('news_keywords');?></th><td><input name="key" type="text" class="input-text" style="width:500px" value="<?php echo ($config['seo_keywords']); ?>" />&nbsp;&nbsp;多个关键词用“,”隔开，字数控制在100个以内</td></tr>
    <tr><th class="text-r"><?php echo C('news_description');?></th><td><textarea name="description"  style="width:500px" class="textarea" /></textarea></td></tr>
    
    
    <tr><th class="text-r"><?php echo C('content');?></th><td>
    <textarea id="content"  name="contents" style="width:100%;height:400px;"></textarea>
    </td></tr>
	<tr>
...
<!--     <tr>
   <th class="text-r">属性选择</th>
    <td>
	<?php if(is_array($attr)): foreach($attr as $key=>$v): ?><input type="checkbox" name="aid[]" value="<?php echo ($v["id"]); ?>" style="margin-left:10px;" ><?php echo ($v["name"]); endforeach; endif; ?>

	</td>
    </tr>
-->    
    <tr><th class="text-r"><?php echo C('status_ok');?></td><td><cite><input name="status" type="radio" value="1" checked="checked" /><?php echo C('yes');?>&nbsp;&nbsp;&nbsp;&nbsp;<input name="status" type="radio" value="0" /><?php echo C('no');?></cite></td></tr>
