...
<body>
	...
	<div id="offCanvasWrapper" class="mui-off-canvas-wrap mui-draggable">
		<!--菜单部分-->
		<aside id="offCanvasSide" class="mui-off-canvas-right" style="overflow-y: auto;">
			<div id="offCanvasSideScroll" class="mui-scroll-wrapper" style="overflow-y: auto;">
				<div  class="mui-scroll" style="overflow-y: auto;">
					<div class="" style="height: 1000px;overflow-y: auto;">
						
					
					<ul id="menulist" class="mui-table-view mui-table-view-chevron mui-table-view-inverted" style="overflow-y: auto;">
						<li class="mui-table-view-cell"><a href="<?php echo U('index/index');?>">网站首页</a></li>
						<?php if(is_array($nav)): $i = 0; $__LIST__ = $nav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li class="mui-table-view-cell  <?php if(!empty($v["child"])): ?>mui-collapse<?php endif; ?>" style="overflow-y: auto;">
							<a class="mui-navigate-right" <?php if(empty($v["child"])): ?>href="<?php echo ($v["url"]); ?>"<?php endif; ?>> <?php echo ($v["name"]); ?></a>
							<?php if(!empty($v["child"])): ?><ul class="mui-table-view mui-table-view-chevron mui-table-view-inverted" style="overflow-y: auto;">
								<?php if(is_array($v["child"])): $i = 0; $__LIST__ = $v["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><li class="mui-table-view-cell" style="overflow-y: auto;">
	...
	<div class="mui-content bg-white">
	    <div class="mui-row" style="margin: 10px -5px 0 0px;">
	    	<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="mui-col-sm-6 mui-col-xs-6 pr-5" >
	        	<a href="<?php echo U('index/show',array('id'=>$v['id']));?>">
	        	<div id="imglist" class="boxshadow" >
					<img src="<?php echo ($v["pic"]); ?>" width="100%" height="220"/>	 
					<p class="mui-ellipsis title text-c"><?php echo ($v["title"]); ?></p>	        		
	        	</div>
	        	</a>
